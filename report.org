...
has a peak memory consumpton around 30 MB (regardless of the amount of
threads launched) and finishes in around 5 seconds in a serial
execution, and one with 1000000 nodes takes 24-25GB of memory and
3h47m to finish in the best case (parallel execution with 8 threads).
...
| / |      <> |       < |     > |       < |     > |       < |     > |
|---+---------+---------+-------+---------+-------+---------+-------|
| # |    1000 | 0:00:04 |  0.03 | 0:00:07 |  0.03 | 0:00:09 |  0.03 |
| # |    5000 | 0:00:28 |  0.13 | 0:00:40 |  0.13 | 0:00:49 |  0.13 |
| # |   10000 | 0:01:03 |  0.25 | 0:01:20 |  0.26 | 0:01:35 |  0.25 |
| # |   50000 | 0:06:20 |  1.24 | 0:07:39 |  1.27 | 0:08:03 |  1.25 |
| # |  100000 | 0:13:34 |  2.47 | 0:15:36 |  2.53 | 0:15:50 |  2.50 |
| # |  300000 | 0:50:58 |  7.38 | 0:55:18 |  7.54 | 0:57:55 |  7.47 |
| # |  500000 | 1:38:16 | 12.30 | 1:34:15 | 12.47 | 1:35:10 | 12.45 |
| # | 1000000 | 4:05:41 | 24.53 | 4:00:42 | 24.89 | 3:47:28 | 24.91 |
|---+---------+---------+-------+---------+-------+---------+-------|
...
TODO: better benchmark for this: To avoid synchro costs, measure
parmap_work() (in master thread and worker threads). That will give
the approximate times of the computation of the processes but without
the synchronization costs.  The only possible problem is how to
implement it: we need an array inside paramap to save the sum of times
of each worker thread and when the SR ends, maestro calculates the
maximum time (and that will be the theoretical time of that SR, since
we are not counting the synchro costs).


TODO: proper use of data to conclude something here.
...
in both Precise mode, and we found that using busy waiters instead of
futexes can result in a speedup between 1.08 and 1.53 using few
threads (up to 8), and from 1.15 to 3 when the amount of threads is
big enough (16 or 24).

Therefore it is recommended to use busy waiters in simulations whose
size is less than 300000 nodes.
...
# This csv is useful for the table of Section 5.2
...
#La idea seria comparar a cada scheduling round y hacer el speedup de cada una. 
# Por ejemplo, la SR tiene 23 procesos, por lo tanto ese speedup va en x=23. En el eje 'y'
# iria el speedup de la comparacion entre la version sequencial y la paralela.
# Problema: NO SE SI SE PUEDE COMPARAR LOS SR DE UNA SECUENCIAL CON UNA PARALELA,
