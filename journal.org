...
** 17 [%]
   - [ ] Try adaptive algorithm with big sizes with optimal threshold (in progress)
   - [ ] It is necessary to run busy/binding with new revision.
   - [ ] Run experiment with all optimizations enabled
         Update data: use new benchmarks (rev 9417357) to compare in adaptive
         (and later in busy, baseline).
