...
# Pastry, Chord, Kademlia).  example="chord"
example='chord'
par_threshold=2
...
for size in "${sizes[@]}"; do
    ...
    for thread in "${threads[@]}"; do
    	...
    	output="thresh${par_threshold}_${size}_threads${thread}_constant.log"
        rm -rf $filename
        ...
        # Gather threshold data from logs
        echo -e '"'$par_threshold'"' >> $output
        grep 'Thres' $filename | awk '{print $(NF)}' >> $output
        $($cp_cmd $output $dest)
    ...
    for thread in "${threads[@]}"; do
    	...
    	output="thresh${par_threshold}_${size}_threads${thread}_precise.log"
