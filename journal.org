...
** 23[%]
   - poner que se optimizaron para tamanios chicos pero que es dificil
     obtener un speedup sobre las cosas que ya andan rapido, es decir,
     lo multithreading a partir de 300k nodos. Poner que el speedup
     obtenido con respecto a las mismas versiones pero sin optimizar
     baja gradualmente a 1, demostrando la dificultad de optimizar
     simulaciones de larga escala con estos metodos.
...
On of Figure \ref{fig:seven}, we can see the speedups achieved
(enabling all optimizations), against the sequential execution.

#+attr_latex: width=0.8\textwidth,placement=[p]
#+label: fig:seven
#+caption: Final speedups achieved binding threads to cores, using adaptive threshold heuristic with optimal initial threshold.
