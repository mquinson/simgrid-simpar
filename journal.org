...
** 14 [%]
  - [ ] Run experiment with all optimizations enabled

  - randomize experiments: instead of explicitly choose the breaks, do some random choice of sizes.
  - Quedarse con el smx_context_rawBESTSOFAR, ahi hay algunas correcciones al viejo.
  ...
  - run all the exps with other examples (kademlia at least)

Other things:
  - amdahl teorethical speedup
  - buen plot de optimal threshold (para encontrar el optimal threshold tambien).
  - hacer ese plot con densidades????
  - final exps with chord/kademlia with all enabled.
  - busy wait falta con 1m, 8 thr. 70k con 24
