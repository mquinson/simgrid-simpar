...
** 18 [0%]
   - [ ] Exp. intrusiveness with 3k, 5k, 10k, 25k, 50k, 75k nodes.
   - [ ] Amdahl speedup with 25k, 50k, 75k, 300k, 500k, 1m.
   - [ ] Core binding exp with 25k, 50k, 75k, and 300k.
   - [ ] Timings with maestrosleeps + (N)
   - [ ] Correr SR en paralelo bien, (fijarse si el secuencial tambien necesita)



 parmap between N cores: maestroawake+(N-1)(default);
                                       ...
                                       the rest involves only creating N threads or N-1 threads:
                                           maestrosleep+(N)
                                           maestro sleep+(N-1)
     I guess I already have this, only need to check when the master is not working. 
