...
To run performance regression test: ./SGXP.pl --site=nancy --cluster=graphene --test=chord,goal --rev="e32a2a561ef184dc9ef8cdaf25759bba6b2ea198,71c9241aa27344f9f8f02f3c1272af4556691713,f95108e7e5bcb66bba76a24c1c433eee710e38e0"
...
** 13 [100%]
   - [X] Performance regression test. repo: git+ssh://rtortilo@scm.gforge.inria.fr//gitroot/simgrid/simgrid-perf-reg-tests.git
         Install r-base ruby gem rserve-client
         To install all libraries of R: first, download them:
         ...
         Then install Rserve (install.package("Rserve") and load library(Rserve) rserve and start Rserve() daemon).
	 After this one should be able to run ./SG-PRT.rb
...
** Optimizations
  1) 
  2) implement parmap between N cores: maestroawake+(N-1)(default?); maestrosleep+(N)(how to sleep master. Check line where master work with its minions and then comment that line? that way master will not work?); maestro sleep+(N-1) (each one of those with busy waiters, futex and posix threads)
