...
** 5 [%]
Report:
   - [ ] Fix bugs of sr-distribution R code
   - [ ] Check the empty figures. Generate new sr-times and sr-distribution.
   - [ ] Keep with the old plot of sr-threshold, its good.
   - [ ] add units/labels to plots. Add conclusions to plots.
   - [ ] Take uniform distribution of sizes (not choose them randomly. Not choose 1000, 5000, 10000. Choose them equally distributed).
   - [ ] Ignore results on emacs maybe will avoid plotting twice some graphs, maybe.

Scripts:
   - [ ] Put host info in each chord_100*
   - [ ] Change scripts to generate csv, not tables.
