...
** 8 [36%]
   - [ ] Go back to the old plot of sr-threshold, its good. (Running new experiments)
   - [ ] Check the empty figures. Generate new sr-times and sr-distribution. (Running new experiments, R code already done)
...
** 9 [%]
   - [ ] Add scripts into file. (testall, testall_sr, get_times)
   - [ ] G5K: add proxy setting (https://www.grid5000.fr/mediawiki/index.php/Getting_Started#Customizing_nodes_and_accessing_the_Internet)
   - [ ] G5K: create an image to accelerate the exp. procedures.

New things:
   - [ ] Investigate about remote file + emacs
