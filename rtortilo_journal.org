...
** 28 [%]
   - [ ] run SR like this: adding all the times of the parallel execution; taking the maxtime of the parallel execution. That way I get the sequential time of each SR, and the parallel time. 
   - [ ] generate 300000, 1000000 and 2000000 nodes files again, there are some problems with the current ones. Run final part of experiments with it(raw, amdahl, sr)
   - [ ] Work on better plots in rstudio. Work on report
   - [ ] Migrate from Rstudio to emacs
   - [ ] Commit fix
