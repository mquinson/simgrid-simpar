...
consequence, this will be our new optimal threshold for parallel
...
fact that we have a lot of SR's with small amount of processes that
are computed sequentially and the fact that we increase the threshold
each time a sequential execution performs better than a parallel).
...
achieved with big size simulations (up to 300000 nodes), which shows the
...
parallel and when to do it in a serial way proved to help.
