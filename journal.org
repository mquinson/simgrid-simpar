...
** 8 [%]
  - [ ] Start Pmap exps.
  - [ ] Start adaptive algorithm experiment with different threshold (try 100,1000,10000).

  - sr-par-threshold: use density function, not dots.
 
  - randomize experiments: instead of explicitly choose the breaks, do some random choice of sizes.
  - Figure 1:
  ...
  - Adaptive algorithm: llevar media de num de procesos que se corren
    en parallelo y sequential.  De esa forma, cuando una cierta
    cantidad no sobrepasa la media, no modificar el threshold, pero
    cuando sobrepasa la media (y queda bajo del treshold actual) medir
    los tiempos y modificar o no el threshold.  De esa forma se
    podrian evitar que numeros chicos de procesos (como 20) afecten el
    treshold.

Maybe is not a good idea, maybe is better to just use the mean. Try to fix it tomorrow
Maybe the problem is with that possible division by zero when calculating averages. I think it never enters that if before the runall_parallel/sequential...

  - instead of calculate the last ratio, better calculate an average
    value of ratios during the simulation. That would solve the
    problems of noise at the end, right?
  - bench 3.11. Does this make sense? because of the performance
    problem that 3.11 has...
  - bench adaptive w/bigger sizes. Keep track of threshold to plot?
    Note: the final threshold depends on the initial one, does it make
    sense to plot it?
