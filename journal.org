...
** 10 [37%]
   ...
   - [ ] Work on portability&reproducibility of report. Rename/cleanup logs folders in a proper way. 
   - [ ] Put host info in each chord_* file.
   - [ ] Try the new amdahl plot.
   - [ ] Put the scripts in Data Generation along with the description and then one
         can choose which one run. (Also put the proper logs/ directories)
 
   - [ ] Run adaptive algorithm with other examples.
   - [ ] Keep thinking about the speedup of SR's data.
   - [ ] run for once and all speedups with 1000000 and 2000000 nodes. (In progress)

   - Ignore results on emacs maybe will avoid plotting twice some graphs, maybe.
* Useful info:
** General setup ("What you have to install in order to run experiments, compile report, etc...")
   - Emacs24
   - org-mode8.2
   - ess
   - R
