...
a deployed node and still be able to setup your environment.

#+name: generate_platforms
#+begin_src python :var list_nb_nodes=[1000] :var nb_bits=32 :var end_date=10000
...
def platform(nb_nodes, nb_bits, end_date):
  ...
  res.append("</platform>")
  res = "".join(res)
  f = open("./Simpar/platform/chord%d.xml"%options.nodes, "w")
  f.write(res)
  f.close()
...
#+begin_src python  :var list_nb_nodes=[1000]
...
<AS  id="AS0"  routing="Full">
  ...
  f = open("./Simpar/deployment/One_cluster_nobb_%d_hosts.xml"%options.nodes, "w")
  f.write(res)
  f.close()
...
cd Simpar/SimGrid/
# Clone latest SimGrid version. You may have to configure proxy settins if you are in a G5K node
...
g<-ggplot(df, aes(x=nodes,y=msteps, group=process, colour=process)) + geom_line() +
   ...
   xlab("Amount of nodes simulated") + ylab("Percentage of SR's")
g
#+end_src
