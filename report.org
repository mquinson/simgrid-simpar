...
the current processes have raised their request and wait for a
...
\cite{previous} or the Documentation section of the official website
of SimGrid \cite{simgrid}.
...
To get a baseline timings and a speedup plot starting from the
development version of SimGrid (3.12), benchmarks to measure the
execution time of a typical Chord simulation in Precise mode with
different amount of threads (1, 2, 4, 8, 16 and 24) were done.

The absolute times of a normal execution for the Chord simulation in the
...
For this section we want to analyze the Scheduling Rounds. The data
...
sequentially. An algorithm to accomplish that is proposed later in
this document.
...
general problem remains: migration of threads between cores entails an
increase of CPU migrations, which in a big size simulation can be
...
The experiments were done using Chord Protocol with Precise mode, but
...
** Dynamic calculation of the optimal threshold
...
why an heuristic for a dynamic threshold calculation is proposed.
...
small sizes, since the algorithm chooses to execute the majority of
...
\State calculate\_current\_avg\_and\_sd\_of\_par\_processes()
...
\State calculate\_current\_avg\_and\_sd\_of\_seq\_processes()
...
general, as it can be seen in Table /cite{tab:two}.
...
The changes proposed can be seen as independent of each other, a fact
...
estimated, along with the adaptive algorithm, physical binding of
threads and busy waiters is then compared to the original times of the
examples to get what we have achieved.

On top of Figure XX(TODO: CREATE FIGURE), we can see the speedups
relatives to the sequential execution, and below, the speedups against
the same parallel versions, but without optimizations enabled.
