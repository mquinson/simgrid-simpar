...
** 17 [0%]
Scripts:
  - [ ] make testall grep important output from logs (dont use the CRITICAl trick anymore)

Experiments:
  - [ ] Make normal speedup exp with default SimGrid. 
    - [ ] After that, try the new amdahl plot with amdahl timings
  - [ ] Improve benchmark of SR's.
  - [ ] Make new SR exp with normal benchmark and uniform distributed
    sizes.

Report:
  - [ ] Add the scripts to org mode.
  - [ ] The Figure 1 should be renamed to "Baseline performance of
    SimGrid 3.11." or something. I would like this graph to:
    - be actually run with SimGrid v3.11 maybe (not sure)
    - have more data (In Progress)
    - report also the memory consumption
    - report some absolute numbers here (Figure 1), maybe on another
      graph so that we can see the time that it actually takes.
  - [ ] Add the used algorithm in section 5.2, as well as absolute
    numbers again, as in 3.1.
