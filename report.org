...
g1<-ggplot(df, aes(x=nodes,y=value, group=threads, colour=threads)) + geom_line() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.title.x = element_blank(), legend.position="none", axis.title.y="Speedup") + ggtitle("Constant Mode")
...
g2<-ggplot(df2, aes(x=nodes,y=value, group=threads, colour=threads)) + geom_line() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.title.x = "Nodes", axis.title.y = "Speedup") + ggtitle("Precise Mode") + scale_fill_discrete(name="threads")
...
#+begin_src R
temp = list.files(path='./logs/sr_counts/new', pattern="threads", full.names = TRUE)
...
for(i in temp){
    ...
    # Yeah, this could be more beautiful...
    ...
    # Populate a new data frame with percentages of interest (1 process, 2 processes, others)
    p <- c(data["1"][[1]],data["2"][[1]], 1 - (data["1"][[1]] + data["2"][[1]]))
    # Calculate the remaining proportion
    #p['others'] <- 1 - (data["1"][[1]] + data["2"][[1]])
    # And bind to existing data.frame
    p <- as.data.frame(p)
    p[,'nodes'] <- nodes
    p[,'process'] <- c("1","2","others")
    proportions <- rbind(proportions, p)
}
df <- ddply(proportions, .(nodes,process), summarise, msteps = mean(p))
g<-ggplot(df, aes(x=nodes,y=msteps, group=process, colour=process)) + geom_line() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.title.x = "Nodes simulated", axis.title.y = "Percentage of SR") + scale_fill_discrete(name="threads")
g
#+end_src
...
g1<-ggplot(df, aes(x=nodes,y=value, group=threads, colour=threads)) + geom_line() + scale_fill_hue() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.x = element_blank(), legend.position="none") +  scale_y_continuous(breaks=c(1,10,20,40)) + ylab("Speedup Constant mode")


g2<-ggplot(df2, aes(x=nodes,y=value, group=threads, colour=threads)) + geom_line() + scale_fill_hue() + theme(axis.text.x = element_text(angle = -45, hjust = 0), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.title.x = "Nodes",  legend.position="bottom") + scale_x_continuous(breaks=c(1000,5000,10000,25000,50000,75000)) +  scale_y_continuous(breaks=c(1,10,20,40)) + ylab("Speedup Precise mode")
...
g1<- ggplot(data=constant, geom="histogram", aes(x=V3, y=speedup)) + xlab("") + ylab("Speedup") +geom_point() + xlim(1,100) + stat_smooth(se=FALSE)

g2<-ggplot(data=precise, geom="histogram", aes(x=V3, y=speedup)) + xlab("Processes in SR") + ylab("Speedup") +geom_point() + xlim(1,100) + stat_smooth(se=FALSE)
