...
** 22 [%]
  - [ ] Absolute numbers in Speedup section and 5.2
  - [ ] The Figure 1 should be renamed to "Baseline performance of
    SimGrid 3.11." or something. Also:
  ...
  - [ ] Make the speedup chart and also the amdahl one. Or put those two in the same graph
Experiments:
  - Make normal speedup exp with default SimGrid. (In
    Progress). Plot.
  - [ ] Improve benchmark of SR's & make new SR exp with normal
    benchmark and uniform distributed sizes.
