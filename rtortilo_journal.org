...
** 17 [%]
*** TODO experiments to check the segfault with big sizes.
**** The problem seems to appear when using benhmarks SR with raw contexts, since with ucontext it does work ok.
***** TODO Check with raw context and no BENCHMARK_PER_SR if it works
****** TODO If it works, then there is a problem catching the context from the list when doing the benchmark.
****** TODO If it does not work, then whe have a problem (This case should not happen)


*** TODO experiments to take mean of values for amdahl for the speedup plot (the same as SR exp, just make more of this and take the mean in R)
*** TODO think experiment of "micro" amdahl timings. That would be I)c)
*** TODO try with other examples.

*** TODO Plot better speedup graph with amdahl law, using bigger sizes (once the segfault is fixed)

*** TODO sequential execution of SR experiments for the plot
